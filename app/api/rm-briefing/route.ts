...
async function fetchBanking(table: string, clientId: string, column = "customer_id") {
  ...
  try {
    ...
    if (error) {
      console.error(`[RM Briefing] Error fetching ${table}:`, error.message)
      return []
    }
    return data ?? []
  } catch (err) {
    console.error(`[RM Briefing] Exception fetching ${table}:`, err)
    return []
  }
}
...
async function fetchCallCenterData(clientId: string, sinceIso: string) {
  try {
    ...
    if (convErr || !conversations?.length) {
      if (convErr) console.error("[RM Briefing] Call center conversations error:", convErr.message)
      return []
    }
    ...
    console.error("[RM Briefing] Call center fetch failed (proceeding without):", err)
    return []
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const systemPrompt = `You are an analyst preparing a proactive briefing for a Relationship Manager (RM) at AIdeology Bank. The RM is about to meet with this client and needs a concise, actionable, insight-led summary.
...
- briefing: 2–3 sentences. Tell the RM the most important things to know about this client right now. Be specific (cite AED figures, product names, recent events).
- main_concern: One sentence. The dominant pain point, worry, or unresolved request — pull from tickets and call center messages.
- next_best_action: One sentence. A concrete RM step (e.g., "Call to confirm card replacement delivery before her London trip on the 24th").
...
Format currency as AED with thousands separators (e.g., AED 1,250,000.00). USD-denominated accounts may show USD.
...
${JSON.stringify(transactions.map((tx: any) => ({ date: tx.date, description: tx.description, amount: tx.amount, type: tx.type, category: tx.category, is_unusual: tx.is_unusual })))}
...
Generate the structured briefing now.`
    ...
    try {
      ...
      console.error("[RM Briefing] Claude generateObject failed:", aiErr)
      return Response.json(
        { error: aiErr?.message || "AI generation failed" },
        { status: 502 },
      )
    }
  } catch (error: any) {
